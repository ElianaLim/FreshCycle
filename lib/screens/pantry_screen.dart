...
class PantryItem {
  ...
  String name;
  String category;
  DateTime expiryDate; // Used when expiryType is absolute
  int relativeDays; // Used when expiryType is relative
  ...
  // Get the actual expiry date (computed from relative days if needed)
  DateTime get computedExpiryDate {
    if (expiryType == ExpiryType.relative) {
      return DateTime.now().add(Duration(days: relativeDays));
    }
    return expiryDate;
  }

  String get daysLeft {
    final diff = computedExpiryDate.difference(DateTime.now()).inDays;
  ...
  String get expiryDisplay {
    if (expiryType == ExpiryType.relative) {
      return 'Expires in $relativeDays day${relativeDays == 1 ? '' : 's'}';
    }
...
class _PantryScreenState extends State<PantryScreen> {
  ...
  double _progressValue(PantryItem item) {
    final daysLeft = item.computedExpiryDate.difference(DateTime.now()).inDays.clamp(0, 9999);
